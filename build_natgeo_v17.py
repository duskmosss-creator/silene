...
print("Building 3-volume National Geographic ZIM collection (v17 - Bulletproof iOS Edition)...")
...
def make_index_html(pdf_list, title, subtitle):
    ...
    for p in pdf_list:
        ...
        cards += f'''
        <a class="card" href="pdfs/{aid}.html" data-href="pdfs/{aid}.html">
...
<script>
    document.querySelectorAll('a[data-href]').forEach(function(el) {{ el.href = el.getAttribute('data-href'); }});
</script>
</body>
</html>"""

def build_vol(pdf_list, vol_num, title, subtitle, name_meta):
    zim_out = f"zim_downloads/National_Geographic_Vol{vol_num}_v17.zim"
    ...
    with Creator(zim_out) as creator:
        creator.add_metadata("Title", title[:28])
        ...
        creator.add_metadata("Description", subtitle[:75])
