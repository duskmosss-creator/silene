...
print("Building v12 National Geographic ZIM volumes...")
...
# Split strictly: Vol 2 = Modern (2000-2024), Vol 1 = Classic & Archival
modern_pdfs  = sorted([p for p in all_pdfs if is_modern(p)], key=lambda p: parse_date(p))
classic_pdfs = sorted([p for p in all_pdfs if not is_modern(p)], key=lambda p: parse_date(p))

print(f"Vol 1 (Classic & Archival): {len(classic_pdfs)} PDFs ({sum(os.path.getsize(p) for p in classic_pdfs)/1024/1024:.0f} MB)")
print(f"Vol 2 (Modern 2009-2019):  {len(modern_pdfs)} PDFs ({sum(os.path.getsize(p) for p in modern_pdfs)/1024/1024:.0f} MB)")
...
def build_vol(pdf_list, vol_num, title, subtitle, name_meta):
    zim_out = f"zim_downloads/National_Geographic_Vol{vol_num}_v12.zim"
...
build_vol(
    ...
    "National Geographic Classic & Archival Collection 1888-1953",
    "nat_geo_vol1_classic_v12"
)
