...
print("Building 3-volume National Geographic ZIM collection (v13)...")
...
for p in all_pdfs:
    ...
    if y < 1916:
        ...
        if len(vol1_pdfs) < len(vol2_pdfs) * 2: vol1_pdfs.append(p)
        else: vol2_pdfs.append(p)

vol1_pdfs.sort(key=lambda p: parse_date(p))
vol2_pdfs.sort(key=lambda p: parse_date(p))
vol3_pdfs.sort(key=lambda p: parse_date(p))

print(f"Vol 1 (1888-1915 Early): {len(vol1_pdfs)} PDFs ({sum(os.path.getsize(p) for p in vol1_pdfs)/1024/1024:.0f} MB)")
print(f"Vol 2 (1916-1953 Mid):   {len(vol2_pdfs)} PDFs ({sum(os.path.getsize(p) for p in vol2_pdfs)/1024/1024:.0f} MB)")
print(f"Vol 3 (2009-2019 Modern):{len(vol3_pdfs)} PDFs ({sum(os.path.getsize(p) for p in vol3_pdfs)/1024/1024:.0f} MB)")
...
def build_vol(pdf_list, vol_num, title, subtitle, name_meta):
    zim_out = f"zim_downloads/National_Geographic_Vol{vol_num}_v13.zim"
    ...
    with Creator(zim_out) as creator:
        ...
        for root, dirs, files in os.walk(natgeo_dir):
            if "texts" in root.split(os.sep): continue
            if "pdfs_original" in root.split(os.sep): continue
            for file in files:
                local_path = os.path.join(root, file)
                zim_path = os.path.relpath(local_path, natgeo_dir).replace("\\", "/")
                if zim_path == "index.html": continue
...
build_vol(
    vol1_pdfs, 1,
    "Nat Geo Vol 1: Early",
    "National Geographic Early Collection 1888-1915",
    "nat_geo_vol1_early_v13"
)

build_vol(
    vol2_pdfs, 2,
    "Nat Geo Vol 2: Mid-Century",
    "National Geographic Mid-Century Collection 1916-1953",
    "nat_geo_vol2_mid_v13"
)

build_vol(
    vol3_pdfs, 3,
    "Nat Geo Vol 3: Modern",
    "National Geographic Modern Collection 2009-2019",
    "nat_geo_vol3_modern_v13"
)
