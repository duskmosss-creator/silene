...
print("Building VOLUMIZED National Geographic ZIM files...")
...
# Parse all PDFs and sort them chronologically
parsed_pdfs = []
for p in pdfs:
...
# Split into Volume 1 (1888-1953) and Volume 2 (2009-2019)
vol1_pdfs = [p for p in parsed_pdfs if p[1] < 2000]
vol2_pdfs = [p for p in parsed_pdfs if p[1] >= 2000 and p[1] != 9999]
# Anything that failed parsing (9999) goes to Vol 1 to keep modern pure
vol1_pdfs.extend([p for p in parsed_pdfs if p[1] == 9999])
...
def generate_index_html(pdf_subset, title, subtitle):
    cards_html = ""
    for p, y, m in pdf_subset:
        aid = get_aid(p)
        
        if y == 9999:
            card_title = f'National Geographic (Archival)'
        else:
            card_title = f'National Geographic<br/>{MONTHS[m]} {y}'
...
<body>
    <div class="header">
        ...
        <p style="color: #94a3b8; margin-top:0.5rem;">{subtitle}</p>
    </div>
    <div class="grid">
        {cards_html}
    </div>
...
def build_zim_vol(pdf_subset, vol_num, title, subtitle):
    zim_filename = os.path.join("zim_downloads", f"National_Geographic_Vol{vol_num}_v10.zim")
    print(f"Building {zim_filename} with {len(pdf_subset)} magazines...")
    
    aids_subset = {get_aid(p) for p, y, m in pdf_subset}
    index_html = generate_index_html(pdf_subset, title, subtitle)
    ...
    with Creator(zim_filename) as creator:
        ...
        creator.add_metadata("Description", subtitle)
        creator.add_metadata("Name", f"national_geographic_vol{vol_num}_v10")
...
# Build Volume 1
build_zim_vol(vol1_pdfs, 1, "National Geographic Vol 1 (1888-1953)", "Classic & Archival Collection")

# Build Volume 2
build_zim_vol(vol2_pdfs, 2, "National Geographic Vol 2 (2009-2019)", "Modern Collection")
